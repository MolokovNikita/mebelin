...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`${config.API_URL}/type-tovara`)
    .then((res)=>{
    let type_tovara = '';
    res.data.forEach((val, indx)=>{
        if(val.name_tovar === category)
          type_tovara = val.id_tovara;
    })
    ...
    const fetchProducts = async () => {
      try {
        ...
        const detailedProducts = await Promise.all(
          productsData.map(async (product) => {
            const [fabricRes, kachRes, typeRes, reviewsRes] = await Promise.all([
              axios.get(`${config.API_URL}/fabric/${product.fabric_id_fabric}`),
              axios.get(`${config.API_URL}/kachestvo/${product.kachestvo_tovara_id_kach}`),
              axios.get(`${config.API_URL}/type-tovara/${product.type_tovara_id_tovara}`),
            ...
            return {
              ...
              score: reviewsRes && reviewsRes.data.length > 0
              ? parseFloat((reviewsRes.data.reduce((acc, val) => {
                  const score = parseFloat(val.score);
                  return acc + (isNaN(score) ? 0 : score);
                }, 0) / reviewsRes.data.length).toFixed(1)) || 0
              : 0,
              scores_count: reviewsRes? reviewsRes.data.length : 0,
              //reviewsRes.data.count || 
              pictures: Array.from({ length: product.pictures_count }, (_, index) => 
        ...
        setCurrentImageIndices(detailedProducts.reduce((acc, product) => {
  ...
  const handlePrevImage = (productId, event) => {
    ...
    const totalImages = products.find((item) => item.id_tovar === productId)?.pictures.length || 0;
  ...
  return (
    <>
      ...
      <span style={{color: "red"}}>Упс, </span>&nbsp;кажется товаров данной категории не существует...
      </div>
      <div className={styles.back_btn}>
        <Link to="/catalog"><button>Вернуться в каталог</button></Link></div>
      </>
      : 
                ...
                <Item className={styles.product__item}>
                  <div className={styles.image__slider}>
                    <ArrowBackIosIcon
                      ...
                      onClick={(event) => handlePrevImage(product.id_tovar, event)}
                    />
                    <img
                      className={styles.product__image}
                      src={
                        product.pictures[currentImageIndices[product.id_tovar] || 0]
                      }
                      alt={`${product.name_tovara}`}
                    />
                    <ArrowForwardIosIcon
                      className={styles.right__arrow}
                      onClick={(event) => handleNextImage(product.id_tovar, event)}
                    />
                  </div>
                  ...
                  <p className={styles.product_desc__name}>{product.name_tovar}</p>
