...
class KachestvoTovaraController {
  ...
  //     const {tovar_type} = req.body; // Например можно получить все товары с типом - 1 - стул
  //     if(!tovar_type){
  ...
  async getOne(req, res) {
    const id_kach = req.params.id;
    const sql = "SELECT * FROM kachestvo_tovara WHERE id_kach = $1";
    pool.query(sql, [id_kach], (err, result) => {
      if (err) {
        console.error(err.message);
...
module.exports = new KachestvoTovaraController();
