...
//   }
  async getAll(req, res) {
        const sql = "SELECT * FROM type_tovara";
  ...
  async getOne(req, res) {
    const id_tovara = req.params.id;
     ...
     const sql = "SELECT * FROM type_tovara WHERE id_tovara = $1";
    pool.query(sql, [id_tovara], (err, result) => {
      if (err) {
        console.error(err.message);
...
module.exports = new TypeTovaraController();
